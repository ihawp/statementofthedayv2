...
function closeSearch() {
    document.querySelector('header').innerHTML = pageInfo['header']['content'];

    // repeated code from printPage();
...
function loadPostsForFollowing() {
    ...
    useAJAXGetData('php/loadPostsFollowing.php', wow)
        .then(posts => {
            printPostContent(posts, 'printPostsSectionnn');
        })
        .catch(error => {
            console.log('Error:', error);
        });
...
function loadPostsForLeaderboardMonthly() {
    useAJAXGetData('php/loadPostsForLeaderboardMonth.php')
        .then(posts => {
            printPostContent(posts, 'leaderboardPostsSectionn');
        })
        .catch(error => {
            console.log('Error:', error);
        });
...
function loadPostsForLeaderboardYearly() {
    useAJAXGetData('php/loadPostsForLeaderboardYear.php')
        .then(posts => {
            printPostContent(posts, 'leaderboardPostsSectionnn');
        })
        .catch(error => {
            console.log('Error:', error);
        });
...
// OPEN
function openSearch() {
    let w = document.querySelector('header');
    w.innerHTML = `
        <div id="alert-container"></div>
        <header>
        <div id="searchFormContainer">
        <button id="closeSearchButton" onclick="closeSearch()"><i class="fa-solid fa-x"></i></button>
            <form id="searchForm">
        ...
        <div id="belowSearchFormContainer">
        </div>
    `;


    document.getElementById('search').addEventListener('keyup', function() {
        let k = document.getElementById('searchForm');
        ...
        useAJAXGetData('php/search.php', wow)
            .then(response=> {

                let afterforeach = ``;
                for (let i = 0; i<response[0].length;) {
                    let w = response[0][i];
                    let k = `
                        <div class="searchResult" onclick="openProfile(${w['id']})">
                            <img src="userPFP/${w['pfp']}" class="searchResultImage">
                            <h1>${w['username']}</h1>
                        </div>
                    `;
                    afterforeach+=k;
                ...
                if (afterforeach !== ``) {
                    document.getElementById('belowSearchFormContainer').innerHTML = `
                    <div id="actualResultPrint">
                    ${afterforeach}
</div>
<div class="spacer"></div>
    ...
    // event listener for typing and get result each time, cancel last result if more stuff typed.
}
...
<button class="postButton" id="makeAPostButton" type="submit"><i class="fa-solid fa-paper-plane"></i></button>
    ...
    if (page === 'leaderboard' || page === 'leaderboardMonth' || page==='leaderboardYear') {
        ...
        let bananaphoneee = null;
        switch (page) {
            case 'leaderboard':
                bananaphoneee = document.getElementById('leaderboardPostsSection');
                break;
            case 'leaderboardMonth':
                bananaphoneee = document.getElementById('leaderboardPostsSectionn');
                break;
            case 'leaderboardYear':
                bananaphoneee = document.getElementById('leaderboardPostsSectionnn');
        }

        bananaphoneee.innerHTML = `
    ...
    if (page === 'settings' && userIDD == getParam('user_id')) {
...
<section id="loginSection">
            ...
            <form method="POST" action="php/login.php" id="loginForm">
                <input type="text" placeholder="username" name="loginusername" id="loginusername" maxlength="12" required>
                <input type="password" placeholder="password" name="loginpassword" id="loginpassword" maxlength="50" required>
        ...
        `
            ...
            <form method="POST" action="php/register.php" id="registerForm">
                <input type="text" placeholder="username" name="username" id="username" maxlength="12" required>
        ...
        `
                ...
                pageInfo = {
                   ...
                   <div id="alert-container">
                            ...
                            <a id="searchButton" onclick="openSearch()"><i class="fa-solid fa-magnifying-glass"></i></a>                      
                ...
                `
                    ...
                    'leaderboardMonth': {
                        'content': `
                            ...
                            <section id="leaderboardPostsSectionn">
                    ...
                    'leaderboardYear': {
                        'content': `
                            ...
                            <section id="leaderboardPostsSectionnn">
                    ...
                    'viewFollowingPosts': {
                        'content': `
                            ...
                            <section id="printPostsSectionnn">
